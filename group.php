...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['first_name'])) {
        ...
        $setting_class->save_settings($_POST, $group_data['userid']);
        }
...
<body style="font-family:tahoma;background-color:#d0d8e4;">
            ...
            <div style="background-color:white; text-align:center; color:#405d9b">
                ...
                <span style="font-size:12px;">
                    ...
                    <?php if (i_own_content($group_data)) : ?>
                        <a onclick="show_change_cover_image(event)" href="<?= ROOT ?>change_profile_image/cover/<?php echo $group_data['userid']; ?>" style="color:#f0f; text-decoration:none;">Change Cover Image</a>
                    <?php endif; ?>
                </span>
                <br>
                    ...
                    <?php if (!group_access($_SESSION['mybook_userid'], $group_data, 'member')) : ?>
                        <?php if (!group_access($_SESSION['mybook_userid'], $group_data, 'request')) : ?>
                            <a href="<?= ROOT ?>join/<?= $group_data['userid'] ?>">
                                <input type="button" id="post_button" value="Join Group" style="margin-right:10px; background-color:#821b93;width:auto;">
                            </a>
                        <?php else : ?>
                            <input type="button" id="post_button" value="Request sent" style="margin-right:10px; background-color:#821b93;width:auto;">
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if (group_access($_SESSION['mybook_userid'], $group_data, 'member')) : ?>

                        <a href="<?= ROOT ?>group/<?php echo $group_data['userid'] ?>/invite">
                            <input type="button" id="post_button" value="Invite" style="margin-right:10px; background-color:#1b9186;width:auto;">
                        </a>
                    <?php endif; ?>
                </div>
                <br>
                ...
                <a href="<?= ROOT ?>group/<?php echo $group_data['userid'] ?>/about">
                    <div id="menu_buttons">About</div>
                </a>
                </a>
                <a href="<?= ROOT ?>group/<?php echo $group_data['userid'] ?>/members">
                    <div id="menu_buttons">Members<?= $members_str ?></div>
                </a>
                <a href="<?= ROOT ?>group/<?php echo $group_data['userid'] ?>/photos">
                ...
                <?php if (group_access($_SESSION['mybook_userid'], $group_data, 'moderator')) : ?>
                    ...
                    ?>
                    <a href="<?= ROOT ?>group/<?php echo $group_data['userid'] ?>/requests">
                        <div id="menu_buttons">Requests<?= $requests_str ?></div>
                    </a>
                <?php endif; ?>

                <?php if (group_access($_SESSION['mybook_userid'], $group_data, 'admin')) : ?>
                    <a href="<?= ROOT ?>group/<?php echo $group_data['userid'] ?>/settings">
    ...
    <?php else : ?>
        <div style="background-color:grey;color:white;padding:1em;text-align:center;margin:1em;">That group was not found</div>
        <br><br>
        <a href="<?= ROOT ?>profile/<?= $_SESSION['mybook_userid'] ?>/groups">Go to groups</a>
