...
function i_own_content($row)
{
    ...
    if (isset($row['postid'])) {
        if ($myid == $row['userid'])
            ...
            //group posts moderator or added admin
            $DB = new Database();
            $groupid = $row['owner'];
...
function notification_seen($id)
{
    ...
    $userid = addslashes($_SESSION['mybook_userid']);
...
function check_messages()
{
    ...
    $query = "select * from messages where (receiver = '$sender' && deleted_receiver = 0 && seen = 0) limit 100";
    $data = $DB->read($query);
    if (is_array($data)) {
        return count($data);
    }
...
function check_seen_thread($msgid)
{
    $DB = new Database();
    $sender = esc($_SESSION['mybook_userid']);
    $query = "select * from messages where (receiver = '$sender' && deleted_receiver = 0 && seen = 0 && msgid = '$msgid') limit 100";
    $data = $DB->read($query);
    if (is_array($data)) {
        return count($data);
    }
